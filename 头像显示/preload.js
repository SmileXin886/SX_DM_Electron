...
// 1. 抹除自动化特征 (必须在 preload 同步执行，否则重载后失效)
Object.defineProperty(navigator, 'webdriver', { get: () => undefined });
Object.defineProperty(navigator, 'plugins', { get: () => [1, 2, 3] });
Object.defineProperty(navigator, 'languages', { get: () => ['zh-CN', 'zh', 'en'] });
window.navigator.chrome = { runtime: {} };

// 2. 还原高价值风控全局变量 (如 __ac_signature) 到 MAIN world
const pageVarsArg = process.argv.find(arg => arg.startsWith('--page-vars='));
if (pageVarsArg) {
    try {
        const pageVarsStr = pageVarsArg.split('=')[1];

        // 核心技术：利用 DOM 挂载 script 标签，突破 contextIsolation 隔离，打入真实的 window 对象
        const script = document.createElement('script');
        script.textContent = `Object.assign(window, ${pageVarsStr});`;
...
contextBridge.exposeInMainWorld('electronAPI', {
     ...
     */
    on: (channel, callback) => {
        const validChannels = ['jimeng:refreshList'];
