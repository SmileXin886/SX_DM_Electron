...
async function startSecureBrowser(credentials) {
    ...
    win.webContents.setWindowOpenHandler(({ url }) => {
        // 只拦截 jimeng 域名内的导航，外部链接忽略
        if (url.includes('jianying.com') || url.includes('toutiao.com') || url.includes('bytedance.com')) {
            win.loadURL(url);
        }
        return { action: 'deny' };
    });
    ...
    win.webContents.on('did-fail-load', (event, errorCode, errorDescription, validatedURL) => {
        // errorCode -3 (ABORTED) 和 -2 (FAILED) 通常是内部子导航被拦截，不是主页面真正失败
