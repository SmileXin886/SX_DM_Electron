...
window.JimengApp = {
    ...
    _renderAccountList() {
        ...
        listEl.innerHTML = accounts.map(acc => {
            const expiresText = acc.createdAt
            ...
            return `
                <div class="jm-account-item" data-account-id="${this._escapeHtml(acc.id)}">
                    <div class="jm-account-info">
                        <div class="jm-account-name">${this._escapeHtml(acc.name || acc.id)}</div>
                        <div class="jm-account-meta">
                            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                <circle cx="12" cy="12" r="10"/>
                                <polyline points="12 6 12 12 16 14"/>
                            </svg>
                            <span>导入于 ${expiresText}</span>
                        </div>
                    </div>
    ...
    async onAccountLaunch(accountId) {
        ...
        this.log('正在启动账号：' + (account?.name || accountId));
