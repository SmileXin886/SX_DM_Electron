...
window.JimengApp = {
     ...
     */
    _renderCredits() {
        const el = document.getElementById('jm-total-credits');
        if (!el) return;
        const credits = JimengState.getTotalCredits();
        el.textContent = credits == null || credits === '' ? '---' : credits;
    },
    ...
    async submitTopup() {
        this.log('触发积分充值...');
        this._showToast('积分系统对接中...', 'warning');
    },
    ...
    async _renderAccountList() {
        ...
        listEl.innerHTML = accounts.map((acc, index) => {
            ...
            // 组装时间显示逻辑
            let timeDisplayStr = '';
            let isPermanent = false; // 【新增】把这个变量提到最外面

            if (acc.isCloud && acc.expireAt) {
                // 1. 格式化激活时间 (基于 createdAt)
                let startStr = '未知';
                let startTime = Date.now(); // 默认保底时间
                if (acc.createdAt) {
                    startTime = acc.createdAt;
                    const startDate = new Date(acc.createdAt);
                    ...
                    startStr = `${startYyyy}/${startMm}/${startDd} ${startHh}:${startMin}`;
                }

                // 2. 判断是否为永久码 (判断标准：有效期超过 10 年，即 3650 天)
                const ONE_YEAR_MS = 365 * 24 * 60 * 60 * 1000;
                isPermanent = (acc.expireAt - startTime) > (10 * ONE_YEAR_MS);

                if (isPermanent) {
                    // 如果是永久码，直接显示不限时长
                    timeDisplayStr = `有效期: ${startStr} - 不限时长`;
                } else {
                    // 3. 正常格式化过期时间
                    ...
                    const expHh = String(expDate.getHours()).padStart(2, '0');
                    const expMin = String(expDate.getMinutes()).padStart(2, '0');
                    const expireStr = `${expYyyy}/${expMm}/${expDd} ${expHh}:${expMin}`;

                    timeDisplayStr = `有效期: ${startStr} - ${expireStr}`;
                }
            } else {
                const importTime = acc.createdAt
                    ? new Date(acc.createdAt).toLocaleString('zh-CN')
                    : '未知';
                timeDisplayStr = `登入于 ${importTime}`;
            }
            ...
            return `
                <div class="jm-account-item" data-account-id="${this._escapeHtml(acc.id)}">
                    ...
                    <div class="jm-account-info ${isPermanent ? 'is-permanent' : ''}">
    ...
    async onAccountLaunch(accountId) {
        ...
        try {
            ...
            if (result.success) {
                this.logSuccess('账号已启动！');
                this._showToast('账号已启动！', 'success');
            ...
            // 【修改这里】：把原来的 this._renderAll(); 换成下面这行
            // 发生错误时（尤其是本地被清理时），强制重新拉取最新列表，同步UI
            this._loadAndRender();
    ...
    // ============================================================
    _showAssetsPage() {
        const mainEl = document.querySelector('.jm-main');
        if (!mainEl) return;

        // 隐藏所有现有卡片
        mainEl.querySelectorAll('.jm-card').forEach(card => {
            card.style.display = 'none';
        });

        // 如果资产管理页面已存在，直接显示
        let assetsPage = document.getElementById('jm-assets-page');
        if (!assetsPage) {
            // 动态加载资产管理页面 HTML
            const tpl = document.getElementById('jm-assets-tpl');
            if (tpl) {
                assetsPage = document.createElement('div');
                assetsPage.innerHTML = tpl.innerHTML;
                mainEl.appendChild(assetsPage.firstElementChild);
                assetsPage = document.getElementById('jm-assets-page');
            }
        }

        if (assetsPage) {
            assetsPage.style.display = '';
            if (window.JimengAssets) window.JimengAssets.init();
        }
    },

    _hideAssetsPage() {
        const assetsPage = document.getElementById('jm-assets-page');
        if (assetsPage) {
    ...
    _bindNavigation() {
        document.querySelectorAll('.jm-nav-item').forEach(item => {
            item.addEventListener('click', () => {
                ...
                if (nav === 'accounts') {
                    this._hideAssetsPage();
                } else if (nav === 'assets') {
                    this._showAssetsPage();
