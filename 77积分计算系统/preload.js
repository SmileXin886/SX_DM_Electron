...
contextBridge.exposeInMainWorld('electronAPI', {
     ...
     * @param {string} licenseKey - 云端激活码
     */
    importJimengCloudAccount: (licenseKey) => ipcRenderer.invoke('jimeng:importCloudAccount', licenseKey),
     ...
     * @param {string} accountId
     * @returns {Promise<{success: boolean, nickname: string, avatarPath: string|null}>}
     */
    getTrackerProfile: (accountId) => ipcRenderer.invoke('tracker:get-profile', accountId),
     ...
     */
    on: (channel, callback) => {
        const validChannels = ['jimeng:refreshList', 'jimeng:windowClosed', 'login:auth-invalid', 'login:silent-verify-start'];
    ...
    /**
     * 验证激活码
     * @param {string} licenseKey - 去除分隔符的激活码
     * @returns {Promise<{success: boolean, message?: string, error?: string}>}
     */
    verifyLicense: (licenseKey) => ipcRenderer.invoke('login:verify', licenseKey),

    /**
     * 通知主进程：激活成功，准备切换到主应用
     * @param {Object} result - 验证结果（可选，供主进程记录日志）
     */
    notifyActivationSuccess: (result) => ipcRenderer.send('login:activation-success', result),
});
