 ...
 * =========================================================
 * 绝对防御版：
 * 1. 修复注入时机导致前几条消息漏网的 Bug。
 * 2. 引入主进程 onBeforeRequest 物理拆包拦截，100% 漏不掉！
 * 3. 完美保留了之前的高级亚克力半透明弹窗和首页判定逻辑。
 */
...
function start(win) {
    ...
    const ses = _win.webContents.session;

    function inject() {
        if (!_win || _win.isDestroyed()) return;
        _win.webContents.executeJavaScript(getInjectionScript()).catch(() => {});
    }
    
    // 【修复漏洞 1】多重生命周期绑定，极速注入，绝不给前端缓存 fetch 的机会！
    _win.webContents.on('dom-ready', inject);
    _win.webContents.on('did-navigate-in-page', inject);
    _win.webContents.on('did-finish-load', inject);
    inject(); // 立即执行一次

    // ==============================================
    // 【修复漏洞 2】物理层拆包安检防线 (抓漏网之鱼)
    // ==============================================
    ses.webRequest.onBeforeRequest(
        { urls: [`*://*.jianying.com/*${TARGET_CONVERSATION_API}*`] },
        (details, callback) => {
            if (!_win || _win.isDestroyed()) return callback({ cancel: false });
            ...
            if (isAgentLeak) {
                ...
                _win.webContents.executeJavaScript(`window.showCanvasBlockAlert && window.showCanvasBlockAlert();`).catch(()=>{});
    ...
    logger.info('画布Agent拦截已启动：多重注入 + 物理拆包安检，一条消息都别想跑！');
}

function stop() {
    _isAttached = false;
    _win = null;
    logger.info('画布拦截器已停止');
}
