 ...
 * 4. CanvasAgentBlocker.start() → 画布模式 Agent 请求拦截（页面注入 + fetch/XHR 劫持）
 *
 * 模块职责边界：
 * - TaskLedger      : 内存状态 + 磁盘持久化 + 双向绑定 + 等待室
 * - HistorySync     : 历史追溯 + 对账 + 积分核算 + UI 推送
 * - CdpInterceptor  : CDP 网络拦截 + 数据提取 + Ledger 操作
 * - CanvasAgentBlocker : 画布模式下 Agent 请求拦截 + Web 弹窗阻断（页面注入版）
 */
...
const AgentReconciler = require('./agent_reconciler');
const CanvasAgentBlocker = require('./canvas_agent_blocker');
...
function attachTracker(parentWin, accountId) {
    ...
    // 4. 初始化 Agent 延迟追溯对账（2分钟轮询）
    AgentReconciler.setPointsUpdateCallback(onPointsUpdate);
    AgentReconciler.start(parentWin);

    // 5. 挂载 CDP 拦截器
    const { detach: detachCdp } = setupCdpInterceptor(parentWin, onPointsUpdate);

    // 6. 启动画布 Agent 拦截器（页面注入版，拦截画布模式下的 Agent 请求）
    CanvasAgentBlocker.start(parentWin);

    // 7. 清理函数
    function destroy() {
        logger.info(`销毁积分追踪器 (accountId=${accountId})`);
        AgentReconciler.stop();
        CanvasAgentBlocker.stop();
        detachCdp();
